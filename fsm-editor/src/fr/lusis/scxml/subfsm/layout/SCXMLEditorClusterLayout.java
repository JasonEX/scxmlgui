...
public class SCXMLEditorClusterLayout extends mxGraphLayout {

	HashSet<String> internalClusterID2DoneLayout; // clusters already done layout
	mxIGraphLayout clusterLayout; // layout to apply to each cluster
	mxGraph graph;

	public SCXMLEditorClusterLayout(mxIGraphLayout cl, mxGraph g) {
		super(g);
		clusterLayout = cl;
	...
	public SCXMLEditorClusterLayout(mxGraph g) {
		super(g);
		graph = g;
	...
	public HashSet<mxCell> findAllClustersRootedAt(mxGraph graph,
			mxCell parent, int depth) {
		return findAllClustersRootedAt(graph, parent, new HashSet<mxCell>(),
				depth);
	}

	public HashSet<mxCell> findAllClustersRootedAt(mxGraph graph,
			mxCell parent, HashSet<mxCell> result, int depth) {
	...
	private void handleLayoutInThisCluster(mxCell cluster,
			HashSet<mxCell> clusters, int depth) {
		...
		if (!internalClusterID2DoneLayout.contains(id)) {
			...
			for (int i = 0; i < numChildren; i++) {
				...
				if (clusters.contains(c)) {
					System.out.println("internal starting layout for cluster: "
							+ c.getValue());
			...
			if (!cluster.isCollapsed()) {
				...
				SCXMLEditorParallelEdgeLayout l1 = new SCXMLEditorParallelEdgeLayout(graph);
				l1.execute(cluster, depth);
				mxEdgeLabelLayout l2 = new mxEdgeLabelLayout(graph);
				l2.execute(cluster);
				graph.updateGroupBounds(new Object[] { cluster },
						2 * graph.getGridSize(), false);
			}
		}
	}
