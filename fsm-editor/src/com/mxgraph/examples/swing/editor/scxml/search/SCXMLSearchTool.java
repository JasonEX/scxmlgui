...
public class SCXMLSearchTool extends JDialog implements ListSelectionListener,
	...
	public SCXMLSearchTool(JFrame parent, SCXMLGraphEditor editor)
	...
	public void buildIndex() throws CorruptIndexException,
			LockObtainFailedException, IOException, SecurityException,
	...
	public void updateCellInIndex(mxCell c, boolean add)
	...
	public void updateCellsInIndex(Collection<mxCell> cs, boolean add)
			throws CorruptIndexException, IOException, ParseException,
			IllegalArgumentException, SecurityException,
			InstantiationException, IllegalAccessException,
			InvocationTargetException, NoSuchMethodException {
	...
	private void populateGUI(JPanel contentPane) {
		...
		searchAndHelp.setLayout(new BoxLayout(searchAndHelp,
				BoxLayout.LINE_AXIS));
	...
	class SCXMLSearchrenderer extends JLabel implements ListCellRenderer {
				...
				boolean cellHasFocus) // the list and the cell have the focus
		{
			...
			if (c.isVertex()) {
				...
				s = e.getSCXMLSource() + "-[" + e.getCondition() + ","
						+ e.getEvent() + "]->" + e.getSCXMLTargets();
			}
			setText(index + ": " + s);
			if (isSelected) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (cmd.equals("help")) {
			new TextDialog(
					this,
					"Search help",
					"The search box accepts Lucene syntax avaialble at:\n"
