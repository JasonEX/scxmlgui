...
public class SCXMLEditorActions {
	...
	public static class AddAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			SCXMLGraphComponent c = (SCXMLGraphComponent) editor
					.getGraphComponent();
			...
			if (!(e.getActionCommand().equals(mxResources.get("addNode")))) {
				...
				for (RestrictedState tempState : editor
						.getRestrictedStatesConfig().getRestrictedState()) {
			...
			mxCell p = (mxCell) graph.insertVertex(parent,
					value.getInternalID(), value, pos.x - parentX, pos.y
							- parentY, 75, 75, value.getStyle());
		}
	}
...
//			JFrame frame = (JFrame) SwingUtilities.windowForComponent(editor);
			...
			try {
				...
				editor.openElementEditorFor(source, Type.OUTGOING_EDGE_ORDER,
						pos);
			} catch (Exception ex) {
				ex.printStackTrace();
			} finally {
...
//			JFrame frame = (JFrame) SwingUtilities.windowForComponent(editor);
			...
			try {
				...
				editor.openElementEditorFor(cell, SCXMLElementEditor.Type.EDGE,
						pos);
			} catch (Exception ex) {
				ex.printStackTrace();
			} finally {
...
//			JFrame frame = (JFrame) SwingUtilities.windowForComponent(editor);
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (editor != null) {
				...
				if (fie instanceof SCXMLImportExport) {
					...
					if (scxmlFie.hasUnhandledXIncludeUsage()
							&& (scxmlFie
									.displayWarningAboutUnhandledXIncludeUsage(
											editor, true) != JOptionPane.YES_OPTION)) {
						return;
					}
				}
				if (isSaveAs) {
					String dirOfLastOpenedFile = editor.menuBar
							.getLastOpenedDir();
					String wd = (lastDir != null) ? lastDir
							: ((editor.getCurrentFile() != null) ? editor
									.getCurrentFile().getParent()
									: ((dirOfLastOpenedFile != null) ? dirOfLastOpenedFile
					...
					if ((prevDate != null)
							...
							&& (JOptionPane.showConfirmDialog(editor,
									mxResources.get("fileModified")) != JOptionPane.YES_OPTION)) {
						return;
					}
				}
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (editor != null) {
				...
				if (AskToSaveIfRequired.check(editor)) {
					...
					SCXMLNode value = (SCXMLNode) editor.getCurrentFileIO()
							.buildNodeValue();
					((SCXMLImportExport) editor.getCurrentFileIO())
							.setRoot(value);
					value.setID(SCXMLNode.ROOTID);
					value.setNamespace("xmlns=\"http://www.w3.org/2005/07/scxml\"");
					value.setCluster(true);
					mxCell p = (mxCell) graph.insertVertex(null,
							value.getInternalID(), value, 0, 0,
							gc.getSize().width, gc.getSize().height,
							value.getStyle());
					p.setValue(value);
	...
	public static class DoLayoutAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			layout.execute((parentToLayout == null) ? graph.getDefaultParent()
					: parentToLayout, depth);
	...
	public static class OpenAction extends AbstractAction {
		...
		public OpenAction(boolean inNewWindow) {
			this(null, inNewWindow);
		}
		...
		private void openInEditor(SCXMLGraphEditor editor) {
			if (editor != null) {
				...
				if (AskToSaveIfRequired.check(editor)) {
					SCXMLFileChoser fc = new SCXMLFileChoser(editor, lastDir,
							file);
					editor.closeAllEditors();
		...
		private void openSelectedFile(SCXMLFileChoser fc,
			...
			if ((graph != null) && (selectedFile != null)) {
				...
				try {
					...
					editor.updateUndoRedoActionState();
					editor.menuBar
							.updateRecentlyOpenedListWithFile(selectedFile);
					editor.getSCXMLSearchTool().buildIndex();

					if (editor.isBackupEnabled())
						IOUtils.copyFile(fc.getSelectedFile(),
								new File(editor.getBackupFileName()));
				} catch (Exception ex) {
					ex.printStackTrace();
					JOptionPane.showMessageDialog(editor.getGraphComponent(),
							ex.toString(), mxResources.get("error"),
	...
	public static class EditSelectedCellAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (cell != null) {
				...
				Point mousePoint = SwingUtilities.convertPoint(gc, screenCoord,
						gc);
				if (cell.isEdge()) {
					...
					new EditNodeAction(cell, root, mousePoint)
							.actionPerformed(e);
				}
			}
			cell = null;
			...
			try {
				...
				if (refresh) {
					if (node.getChildCount() > 0) {
						editor.displayOutsourcedContentInNode(node, graph,
								false, false);
						editor.displayOutsourcedContentInNode(node, graph,
								true, true);
					}
...
//			mxIGraphModel model = graph.getModel();
			try {
				...
				editor.setDisplayOfOutsourcedContentSelected(!editor
						.isDisplayOfOutsourcedContentSelected());
			} catch (Exception e1) {
				e1.printStackTrace();
			}
			editor.getUndoManager().setCollectionMode(false);
	...
	public static void convertNoGUI(SCXMLGraphEditor editor) throws Exception {
		...
		if (format.equalsIgnoreCase("dot")) {
			...
			fie.write(graphComponent, output, format,
	...
	public static class SCXMLDelete extends DeleteAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			SCXMLGraphComponent graphComponent = editor.getGraphComponent();
			SCXMLGraph graph = graphComponent.getGraph();
			Object[] cells = graph.getDeletableCells(graph.addAllEdges(graph
					.getSelectionCells()));
