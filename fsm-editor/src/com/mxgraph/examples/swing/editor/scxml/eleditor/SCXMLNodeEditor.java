...
public class SCXMLNodeEditor extends SCXMLElementEditor {
	...
	public SCXMLNodeEditor(JFrame parent, mxCell nn, mxCell rootOfGraph,
			SCXMLGraphEditor editor, Point pos) {
		...
		if (nn != rootOfGraph) {
			...
			if (!node.isHistoryNode() && !node.getFake()) {
				...
				onentryTextPane = new UndoJTextPane(node.getOnEntry(), doc,
						undo, keyboardHandler);
				if (doc == null) {
					node.setOnEntryDoc(doc = onentryTextPane.getDocument());
					node.setOnEntryUndoManager(undo = onentryTextPane
							.getUndoManager());
				}
				...
				if (doc == null) {
					...
					node.setOnExitUndoManager(undo = onexitTextPane
							.getUndoManager());
				}
			...
			if (node.isFinal() && !node.getFake()) {
				...
				finalTextPane = new UndoJTextPane(node.getDoneData(), doc,
						undo, keyboardHandler);
				if (doc == null) {
					node.setDoneDataDoc(doc = finalTextPane.getDocument());
					node.setDoneDataUndoManager(undo = finalTextPane
							.getUndoManager());
				}
			...
			if (node.isInitial() && !node.getFake()
					&& (nnParent != rootOfGraph)) {
				undo = node.getOnInitialEntryUndoManager();
				doc = node.getOnInitialEntryDoc();
				initialTextPane = new UndoJTextPane(node.getOnInitialEntry(),
						doc, undo, keyboardHandler);
				if (doc == null) {
					node.setOnInitialEntryDoc(doc = initialTextPane
							.getDocument());
					node.setOnInitialEntryUndoManager(undo = initialTextPane
							.getUndoManager());
				}
				...
				scrollPane.setPreferredSize(new Dimension(400, 200));
				tabbedPane.addTab(mxResources.get("initialEntryTAB"),
						scrollPane);
				doc.addDocumentListener(changeListener);
			}
		} else {
		...
		if (!node.isHistoryNode() && !node.getFake()) {
			...
			datamodelPane = new UndoJTextPane(node.getDatamodel(), doc, undo,
					keyboardHandler);
			if (doc == null) {
				node.setDatamodelDoc(doc = datamodelPane.getDocument());
				node.setDatamodelUndoManager(undo = datamodelPane
						.getUndoManager());
			}
			...
			namespacePane = new UndoJTextPane(node.getNamespace(), doc, undo,
					keyboardHandler);
			if (doc == null) {
				node.setNamespaceDoc(doc = namespacePane.getDocument());
				node.setNamespaceUndoManager(undo = namespacePane
						.getUndoManager());
			}
		...
		if (nn != rootOfGraph) {
			...
			commentsPane = new UndoJTextPane(node.getComments(), doc, undo,
					keyboardHandler);
			if (doc == null) {
				node.setCommentsDoc(doc = commentsPane.getDocument());
				node.setCommentsUndoManager(undo = commentsPane
						.getUndoManager());
			}
		...
		scriptPane = new UndoJTextPane(node.getScript(), doc, undo,
				keyboardHandler);
		if (doc == null) {
