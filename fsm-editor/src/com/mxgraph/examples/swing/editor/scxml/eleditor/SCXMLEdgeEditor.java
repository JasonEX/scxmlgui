...
public class SCXMLEdgeEditor extends SCXMLElementEditor {
	...
	public SCXMLEdgeEditor(JFrame parent, mxCell en, SCXMLGraphEditor editor,
			Point pos) {
		...
		if (doc == null) {
			...
			edge.setConditionUndoManager(undo = conditionTextPane
					.getUndoManager());
		}
		doc.addDocumentListener(changeListener);
		...
		commentsPane = new UndoJTextPane(edge.getComments(), doc, undo,
				keyboardHandler);
		if (doc == null) {
		...
		sourceNode = (SCXMLNode) editor.getGraphComponent()
				.getSCXMLNodeForID(edge.getSCXMLSource()).getValue();
		if (sourceNode.isRestricted()) {
			...
			buttonGroupScrollPane = new JScrollPane(
					possibleEventsButtonGroupPanel);
			...
			JLabel eventNameTitleLabel = new JLabel(
					mxResources.get("eventNameTitle"));
			...
			JLabel evenDocumentationTitleLabel = new JLabel(
					mxResources.get("eventDocumentationTitle"));
			c = new GridBagConstraints();
			c.fill = GridBagConstraints.HORIZONTAL;
			c.weightx = 0.5;
			c.weighty = 0;
			...
			JScrollPane documentationScrollPane = new JScrollPane(
					eventDocumentationLabel);
			// documentationScrollPane.setPreferredSize(new Dimension(400,
			// 200));
			...
			gbl.columnWidths = new int[] {
					buttonGroupScrollPane.getPreferredSize().width + 100, 600 };

			restrictedEdgeEditorPanel.validate();

			tabbedPane.addTab(mxResources.get("eventTAB"),
					restrictedEdgeEditorPanel);
		} else {
	...
	private void loadPossibleEventsButtonGroup(SCXMLGraphEditor editor) {
		...
		for (PossibleEvent possibleEvent : sourceNode.getPossibleEvents()) {
			...
			possibleEventRadioButton.setActionCommand(mxResources
					.get("changeEvent")
					+ eventName
					+ "#&@"
					+ possibleEvent.getDocumentation());
			...
			if (eventName.equals(edge.getEvent())) {
				...
				possibleEventRadioButton.setEnabled(!existingEventsOnSourceNode
						.contains(eventName));
			}
			eventButtonGroup.add(possibleEventRadioButton);
			// possibleEventRadioButton.setPreferredSize(new Dimension(180,
			// 25));
			GridBagConstraints c = new GridBagConstraints();
