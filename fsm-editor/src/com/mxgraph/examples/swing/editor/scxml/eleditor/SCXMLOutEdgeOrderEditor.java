...
public class SCXMLOutEdgeOrderEditor extends JDialog implements
	...
	public SCXMLOutEdgeOrderEditor(JFrame parent, mxCell source,
			SCXMLGraphEditor e, Point pos) {
		super(parent, true);
		editor = e;
		...
		updateInputMap(getRootPane().getInputMap(
	...
	public class CloseAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			SCXMLOutEdgeOrderEditor.this.actionPerformed(new ActionEvent(this,
					0, "cancel"));
			editor.setEditorForCellAndType(
					cell,
					com.mxgraph.examples.swing.editor.scxml.eleditor.SCXMLElementEditor.Type.OUTGOING_EDGE_ORDER,
					null);
		}
	}
	...
	private void populateGUI(JPanel contentPane) {
		...
		moveButtonPane.setLayout(new BoxLayout(moveButtonPane,
				BoxLayout.LINE_AXIS));
		...
		closeButtonPane.setLayout(new BoxLayout(closeButtonPane,
				BoxLayout.LINE_AXIS));
	...
	class EdgeRenderer extends JLabel implements ListCellRenderer {
				...
				boolean cellHasFocus) // the list and the cell have the focus
		{
			...
			if (value != null) {
				...
				text = v.getOrder() + ": -> "
						+ graph.convertValueToString(e.getTarget()) + " ["
						+ graph.convertValueToString(e) + "]";
			}
			setText(text);
			if (isSelected) {
	...
	public void valueChanged(ListSelectionEvent e) {
		if (e.getValueIsAdjusting() == false) {
			if (list.isSelectionEmpty()) {
				...
				Integer max = max(sis);
				Integer min = min(sis);
				if ((max != null) && (max < listModel.size() - 1))
			...
			if (!getModified())
				...
				case JOptionPane.YES_OPTION:
					...
					model.fireEvent(new mxEventObject(mxEvent.CHANGE,
							"changes", changes, "revalidate", false));
					exitTool();
					break;
				}
			}
