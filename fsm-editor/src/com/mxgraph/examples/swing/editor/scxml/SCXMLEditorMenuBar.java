...
public class SCXMLEditorMenuBar extends JMenuBar {
	...
	public void getRecentlyOpenedFiles(final SCXMLGraphEditor editor) {
		...
		for (int i = 0; i < MAX_RECENT_FILE_HISTORY; i++) {
			String fileName = editor.preferences.get(PREFERENCE_FILES_KEY + i,
					null);
			if (fileName != null)
	...
	public void updateRecentlyOpenedListWithFile(File file) {
		...
		if (file.exists())
			editor.preferences.put(PREFERENCE_LASTFILE_KEY, fileName);
		if (!recentlyOpenedFiles.contains(fileName)) {
			...
			for (String fn : recentlyOpenedFiles) {
				if (new File(fn).exists())
					editor.preferences.put(PREFERENCE_FILES_KEY + (i++), fn);
			}
	...
	public SCXMLEditorMenuBar(final SCXMLGraphEditor ed) {
		...
		Action externalAction = editor.bind(mxResources.get("undo"),
				internalAction, "/com/mxgraph/examples/swing/images/undo.gif");
		...
		menu.add(editor.bind(mxResources.get("find"), new ShowSCXMLFindTool(),
				"/com/mxgraph/examples/swing/images/zoom.gif"));
		...
		menu.add(editor.bind(mxResources.get("showSCXMLListener"),
				new ShowSCXMLListener()));

		menu = add(new JMenu(mxResources.get("view")));
		menu.setMnemonic(KeyEvent.VK_V);
		internalAction = new ToggleDisplayOutsourcedContent();
		JCheckBoxMenuItem menuItem = new JCheckBoxMenuItem(
				externalAction = editor.bind(
						mxResources.get("toggleDisplayContentOutsourced"),
						internalAction));
		menu.add(menuItem);
		...
		JMenuItem item = menu.add(new JMenuItem(mxResources
				.get("aboutGraphEditor")));
	...
	private void makeFileMenu(JMenu menu, SCXMLGraphEditor editor) {
		...
		menu.add(editor.bind(mxResources.get("openFileInNewWindow"),
				new OpenAction(true)));
		JCheckBoxMenuItem menuItem = new JCheckBoxMenuItem(editor.bind(
				mxResources.get("ignoreStoredLayout"),
				new ToggleIgnoreStoredLayout()));
		...
		if (!recentlyOpenedFiles.isEmpty()) {
			...
			for (String fileName : recentlyOpenedFiles) {
				File file = new File(fileName);
				if (file.exists()) {
					Action itemAction = editor.bind(fileName, new OpenAction(
							file));
