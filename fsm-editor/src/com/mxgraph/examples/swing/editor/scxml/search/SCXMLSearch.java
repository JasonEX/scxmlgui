...
public class SCXMLSearch {
	...
	public void buildIndex() throws CorruptIndexException,
			LockObtainFailedException, IOException, ClassNotFoundException,
			SecurityException, NoSuchMethodException, IllegalArgumentException,
			InstantiationException, IllegalAccessException,
			InvocationTargetException {
		SCXMLGraph graph = editor.getGraphComponent().getGraph();
		analyzer = Class
				.forName("com.mxgraph.examples.swing.editor.scxml.SCXMLAnalyzer");
		...
		IndexWriter writer = new IndexWriter(idx, new IndexWriterConfig(
				Version.LUCENE_36, (Analyzer) constructor.newInstance()));
		writer.deleteAll();
		writer.commit();
	...
	private Document createDocumentForSCXMLEdge(SCXMLEdge v, String cellID) {
		...
		doc.add(new Field(INDEXID, cellID, Field.Store.YES,
				Field.Index.ANALYZED));
		doc.add(new Field("source", new StringReader(v.getSCXMLSource()
				.toLowerCase())));
		doc.add(new Field("target", new StringReader(v.getSCXMLTargets()
		...
		doc.add(new Field("com",
				new StringReader(v.getComments().toLowerCase())));
		doc.add(new Field("all", new StringReader((v.getSCXMLSource() + " "
				+ v.getSCXMLTargets().toString() + " " + v.getEvent() + " "
				+ v.getCondition() + " " + v.getExe() + " " + v.getComments())
				.toLowerCase())));
		return doc;
	}
	...
	private Document createDocumentForSCXMLNode(SCXMLNode v, String cellID) {
		...
		doc.add(new Field("inc", new StringReader(v.getOutsourcedLocation()
				.toLowerCase())));
		doc.add(new Field("dm",
				new StringReader(v.getDatamodel().toLowerCase())));
		doc.add(new Field("ns",
				new StringReader(v.getNamespace().toLowerCase())));
		...
		doc.add(new Field("com",
				new StringReader(v.getComments().toLowerCase())));
		doc.add(new Field("all", new StringReader((v.getID() + " "
				+ v.getOutsourcedLocation() + " " + v.getDatamodel() + " "
				+ v.getNamespace() + " " + v.getOnEntry() + " " + v.getOnExit()
				+ " " + v.getOnInitialEntry() + " " + v.getDoneData() + " " + v
				.getComments()).toLowerCase())));
		return doc;
	}
	...
	private Document createDocumentForCell(mxCell c) {
		...
		Object v = c.getValue();
		Document doc = (c.isVertex()) ? createDocumentForSCXMLNode(
				(SCXMLNode) v, cellID) : createDocumentForSCXMLEdge(
				(SCXMLEdge) v, cellID);
		return doc;
	}

	public void updateIndex(Collection<mxCell> cs, boolean add)
	...
	public Query getQueryForGettingDocumentOfCell(mxCell c)
	...
	public ArrayList<mxCell> find(String query) throws IOException,
			IllegalArgumentException, InstantiationException,
			IllegalAccessException, InvocationTargetException,
			SecurityException, NoSuchMethodException {
		Constructor constructor = analyzer.getConstructor();
		QueryParser queryParser = new QueryParser(Version.LUCENE_30, "all",
				(Analyzer) constructor.newInstance());
		try {
	...
	public void printResult(TopDocs result) throws CorruptIndexException,
			IOException {
		int numHits = result.totalHits;
		...
		for (int i = 0; i < numHits; i++) {
			...
			if (matchingCell != null) {
				System.out
						.println(" search result: " + matchingCell.getValue());
			}
		}
	}
