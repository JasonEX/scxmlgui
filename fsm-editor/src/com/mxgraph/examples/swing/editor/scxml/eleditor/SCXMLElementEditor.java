...
public class SCXMLElementEditor extends JDialog implements ActionListener,
	...
	private AbstractActionWrapper externalUndoAction = null,
	...
	public SCXMLElementEditor(JFrame parent, SCXMLGraphEditor e, mxCell cell) {
		...
		editor = e;
		externalUndoAction = editor.bind(mxResources.get("undo"), null,
				"/com/mxgraph/examples/swing/images/undo.gif");
		externalRedoAction = editor.bind(mxResources.get("redo"), null,
	...
	protected HashMap<Object, Action> updateActionTable(JTabbedPane tabbedPane,
	...
	private void setActionsForUndoJTextPane(Component o,
			HashMap<Object, Action> actions) {
		UndoJTextPane u;
		u = (UndoJTextPane) o;
		...
		if ((externalUndoAction != null) && (externalRedoAction != null)) {
			if (ua.getExternalAction() == null)
				ua.setExternalAction(externalUndoAction);
			if (ra.getExternalAction() == null)
				ra.setExternalAction(externalRedoAction);
		...
		if (keyboardHandler != null)
			keyboardHandler.updateActionMap();
	}

	private void setActionsForUndoJTextField(Component o,
			HashMap<Object, Action> actions) {
		UndoJTextField u;
		u = (UndoJTextField) o;
		ActionMap actionMap = u.getActionMap();
	...
	private void setDefaultActions(HashMap<Object, Action> actions,
			ActionMap actionMap) {
		actions.put(DefaultEditorKit.copyAction,
				actionMap.get(DefaultEditorKit.copyAction));
		actions.put(DefaultEditorKit.cutAction,
				actionMap.get(DefaultEditorKit.cutAction));
		actions.put(DefaultEditorKit.pasteAction,
				actionMap.get(DefaultEditorKit.pasteAction));
		actions.put(DefaultEditorKit.selectAllAction,
	...
	public Type getTypeForEditorClass() {
		...
		if (cn.equals(SCXMLEdgeEditor.class.getName()))
			return Type.EDGE;
		else if (cn.equals(SCXMLNodeEditor.class.getName()))
			return Type.NODE;
		else if (cn.equals(SCXMLOutsourcingEditor.class.getName()))
	...
	protected class DocumentChangeListener implements DocumentListener {
		...
		public void insertUpdate(DocumentEvent e) {
			mxGraphModel model = (mxGraphModel) editor.getGraphComponent()
					.getGraph().getModel();
			model.fireEvent(new mxEventObject(mxEvent.CHANGE, "changes",
					changes, "revalidate", false));
		}
	...
	protected JMenu createEditMenu() {
		...
		menu.add(editor.bind(mxResources.get("selectAll"),
				getActionByName(DefaultEditorKit.selectAllAction), null));
		return menu;
	}
