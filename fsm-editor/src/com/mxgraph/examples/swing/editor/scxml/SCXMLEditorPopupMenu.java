...
public class SCXMLEditorPopupMenu extends JPopupMenu {
	...
	public SCXMLEditorPopupMenu(SCXMLGraphEditor editor, Point mousePt,
			Point graphPt, Point screenCoord) {
			...
			if (c.isEdge()) {
				...
				if (!inOutsourcedNode) {
					...
					if (!((source != null) && (source.isHistoryNode()))) {
						add(editor.bind(mxResources.get("editEdge"),
								new EditEdgeAction(c, screenCoord)));
						addSeparator();
					}
					...
					if (c.getSource() != c.getTarget()) {
						...
						if ((index = cellState.getIndexOfEdgePointAt(graphPt.x,
								graphPt.y, gc.getTolerance())) == -1) {
							int indexOfNewPoint = cellState.getIndexOfNewPoint(
									graphPt.x, graphPt.y, gc.getTolerance()) - 1;
							if (indexOfNewPoint >= 0) {
								add(editor.bind(mxResources.get("addCorner"),
										new AddCornerToEdgeAction(c,
												unscaledGraphPoint, graphPt,
												indexOfNewPoint)));
							}
						...
						JCheckBoxMenuItem menuItem = new JCheckBoxMenuItem(
								editor.bind(
										mxResources.get("toggleWithTarget"),
										new ToggleWithTargetAction(c)));
						menuItem.setSelected(((SCXMLEdge) (c.getValue()))
								.isCycleWithTarget());
						add(menuItem);
					}
				}
			} else if (c.isVertex()) {
				boolean nodeIsOutsourced = ((SCXMLNode) (c.getValue()))
						.isOutsourcedNode();
				boolean nodeIsFake = ((SCXMLNode) (c.getValue())).getFake();
				mxICell parent = c.getParent();
				boolean isHistoryNode = ((SCXMLNode) (c.getValue()))
						.isHistoryNode();
				boolean isParallelNode = ((SCXMLNode) (c.getValue()))
						.isParallel();
				boolean isFinalNode = ((SCXMLNode) (c.getValue())).isFinal();
				boolean isClusterNode = ((SCXMLNode) (c.getValue()))
						.isClusterNode();
				if (!inOutsourcedNode) {
					// add node in case the cell under the pointer is a swimlane
					boolean addNodeEnabled = graph.isSwimlane(c)
							&& (editor.getCurrentFileIO() != null);
					add(
							editor.bind(mxResources.get("addNode"),
									new AddAction(mousePt, c))).setEnabled(
							addNodeEnabled);
					/*
					 * Add new, restricted node
					 */
					if (editor.getRestrictedStatesConfig() != null) {
						JMenu addRestrictedNodeMenu = new JMenu(
								mxResources.get("addRestrictedNode"));
						addRestrictedNodeMenu.setEnabled(addNodeEnabled);
						List<RestrictedState> restrictedStatesList = editor
						...
						if (restrictedStatesList != null) {
							for (RestrictedState restrictedState : restrictedStatesList) {
								JMenuItem addRestrictedStateMenuItem = new JMenuItem(
										editor.bind(restrictedState.getName(),
												new AddAction(mousePt, c)));
								addRestrictedStateMenuItem
										.setEnabled(addNodeEnabled);
								addRestrictedStateMenuItem
										.setToolTipText(restrictedState
												.getDocumentation());
								addRestrictedNodeMenu
										.add(addRestrictedStateMenuItem);
							}
						}
					...
					add(editor.bind(mxResources.get("editNode"),
							new EditNodeAction(c, root, screenCoord)));
					if (c != root) {
						if (!isHistoryNode) {
							if (!nodeIsFake)
								add(
										editor.bind(mxResources
												.get("editOutgoingEdgeOrder"),
							...
							menuItem2.setEnabled(!((SCXMLNode) (c.getValue()))
									.isClusterNode()
									|| (c.getChildCount() == 0)
									|| ((SCXMLNode) (c.getValue()))
											.isOutsourcedNode());
							add(menuItem2);
						}
						if (!nodeIsFake) {
							...
							JCheckBoxMenuItem menuItem = new JCheckBoxMenuItem(
									editor.bind(
											mxResources.get("setAsInitialNode"),
											new SetNodeAsInitial(c)));
							menuItem.setSelected(((SCXMLNode) (c.getValue()))
									.isInitial());
							add(menuItem);
							if (!isHistoryNode) {
								menuItem = new JCheckBoxMenuItem(editor.bind(
										mxResources.get("setAsFinalNode"),
										new SetNodeAsFinal(c)));
								menuItem.setSelected(((SCXMLNode) (c.getValue()))
										.isFinal());
								menuItem.setEnabled(!((SCXMLNode) (c.getValue()))
										.isRestricted());
								add(menuItem);
								menuItem = new JCheckBoxMenuItem(editor.bind(
										mxResources.get("setAsClusterNode"),
										new SetNodeAsCluster(c)));
								menuItem.setSelected(((SCXMLNode) (c.getValue()))
										.isClusterNode());
								menuItem.setEnabled(!((SCXMLNode) (c.getValue()))
										.isOutsourcedNode());
								add(menuItem);
								menuItem = new JCheckBoxMenuItem(editor.bind(
										mxResources.get("setAsParallelNode"),
										new SetNodeAsParallel(c)));
								menuItem.setSelected(((SCXMLNode) (c.getValue()))
										.isParallel());
								add(menuItem);
								 ...
								 */
								if (editor.getRestrictedStatesConfig() != null) {
									JMenu toggleRestrictedMenu = new JMenu(
											mxResources
													.get("setAsRestrictedNode"));
									List<RestrictedState> restrictedStatesList = editor
											.getRestrictedStatesConfig()
											.getRestrictedState();
									SCXMLNode tempNode = (SCXMLNode) c
											.getValue();
									toggleRestrictedMenu.setEnabled(!tempNode
											.isFinal());
									if (restrictedStatesList != null) {
										for (RestrictedState restrictedState : restrictedStatesList) {
											menuItem = new JCheckBoxMenuItem(
													editor.bind(
															restrictedState
																	.getName(),
															new SetNodeAsRestricted(
																	c)));
											menuItem.setToolTipText(restrictedState
													.getDocumentation());
											menuItem.setSelected(tempNode
													.isRestricted(restrictedState));
											menuItem.setEnabled(!tempNode
													.isFinal());
							...
							if ((parent != root)
									&& ((SCXMLNode) parent.getValue())
											.isClusterNode()) {
								if (!isParallelNode && !isClusterNode
										&& !isFinalNode) {
									addSeparator();
									menuItem = new JCheckBoxMenuItem(
											editor.bind(
													mxResources
															.get("setAsDeepHistoryNode"),
													new SetNodeAsHistory(c,
															true)));
									menuItem.setSelected(((SCXMLNode) (c
											.getValue())).isDeepHistory());
									add(menuItem);
									menuItem = new JCheckBoxMenuItem(
											editor.bind(
													mxResources
															.get("setAsShallowHistoryNode"),
													new SetNodeAsHistory(c,
															false)));
									menuItem.setSelected(((SCXMLNode) (c
											.getValue())).isShallowHistory());
									add(menuItem);
								}
							}
						}
				...
				if (nodeIsOutsourced) {
					JCheckBoxMenuItem menuItem = new JCheckBoxMenuItem(
							editor.bind(mxResources
									.get("toggleViewOutsourcedContent"),
					...
					if (c.getChildCount() > 0) {
						JMenuItem itemMenu = new JMenuItem(
								editor.bind(
										mxResources
												.get("refreshViewOutsourcedContent"),
										new ToggleDisplayOutsourcedContentInNode(
												c, true)));
						add(itemMenu);
					}
					...
					File f = editor.getThisFileInCurrentDirectory(src
							.getLocation());
					Action itemAction = editor.bind(
							mxResources.get("openInNewWindow") + " "
									+ f.getName(), new OpenAction(f, true));
					JMenuItem itemMenu = new JMenuItem(itemAction);
					add(itemMenu);
				}
				addSeparator();
				add(editor.bind(mxResources.get("doRecursiveLayout"),
						new DoLayoutAction(graph, c, -1)));
				add(editor.bind(mxResources.get("doSimpleLayout"),
						new DoLayoutAction(graph, c, 0)));
			}
		} else {
			add(editor.bind(mxResources.get("editNodeEdge"), null)).setEnabled(
					false);
		}
	}
}
