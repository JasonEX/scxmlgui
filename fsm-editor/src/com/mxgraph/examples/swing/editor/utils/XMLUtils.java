...
public class XMLUtils {
	...
	public static String domNode2String(Node node, boolean escapeStrings) {
		...
		case Node.ELEMENT_NODE:
			...
			for (int i = 0; i < attributes.getLength(); i++) {
				...
				ret += " "
						+ current.getNodeName()
						+ "=\""
						+ ((escapeStrings) ? escapeStringForXML(current
								.getNodeValue()) : current.getNodeValue())
						+ "\"";
			}
			ret += ">";
		...
		case Node.TEXT_NODE:
			ret += (escapeStrings) ? escapeStringForXML(node.getNodeValue())
					: node.getNodeValue();
			break;
	...
	private static String prettyPrintDom(Node node, String indent,
			boolean isRoot, boolean escapeStrings) {
		String ret = "";
		switch (node.getNodeType()) {
		...
		case Node.ELEMENT_NODE:
			...
			if (children != null) {
				for (int i = 0; i < children.getLength(); i++) {
					String tmp = prettyPrintDom(children.item(i), indent
							+ ((isRoot) ? "" : BI), false, escapeStrings);
	...
	public static String prettyPrintXMLString(String xml, String indent,
			boolean escapeStrings) throws Exception {
		if (xml == null)
			return null;
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println(prettyPrintXMLString(a, " ", true));
			Document doc = mxXmlUtils.parseXml(a);
			String b = prettyPrintDom(doc, "", true, true);
			System.out.println(b);

			System.out.println(c);
			System.out.println(prettyPrintXMLString(c, " ", true));
			doc = mxXmlUtils.parseXml(c);
			b = prettyPrintDom(doc, "", true, true);
			System.out.println(b);

			System.out.println(d);
			System.out.println(prettyPrintXMLString(d, " ", true));
			doc = mxXmlUtils.parseXml(d);
			b = prettyPrintDom(doc, "", true, true);
			System.out.println(b);

			System.out.println(e);
			System.out.println(prettyPrintXMLString(e, " ", true));
			doc = mxXmlUtils.parseXml(e);
			b = prettyPrintDom(doc, "", true, true);
			System.out.println(b);
		} catch (Exception e1) {
			e1.printStackTrace();
	...
	public static String isParsableXMLString(String xml) {
		try {
			...
			docBuilder.parse(new InputSource(new StringReader("<xml>" + xml
					+ "</xml>")));
