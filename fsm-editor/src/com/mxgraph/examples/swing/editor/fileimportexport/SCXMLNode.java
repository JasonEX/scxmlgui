...
public class SCXMLNode implements Serializable {
	...
	public void setParallel(boolean b) {
		setStrokeColor((isFinal()) ? FINALSTROKECOLOR
				: ((b) ? PARALLELSTROKECOLOR : DEFAULTSTROKECOLOR));
	...
	public Boolean isDeepHistory() {
		return ((node.get(HISTORY) != null) && (node.get(HISTORY)
				.equals(HISTORYTYPE.DEEP)));
	}

	public Boolean isShallowHistory() {
		return ((node.get(HISTORY) != null) && (node.get(HISTORY)
				.equals(HISTORYTYPE.SHALLOW)));
	}
	...
	public void setFinal(Boolean b) {
		setStrokeColor((b) ? FINALSTROKECOLOR
				: ((isParallel()) ? PARALLELSTROKECOLOR : DEFAULTSTROKECOLOR));
		node.put(FINAL, b);
	}
	...
	public String getStyle() {
		...
		for (String k : sh.keySet()) {
			if (!k.equals("root")) {
				if (!outSourced
						|| (!k.equals(mxConstants.STYLE_STROKEWIDTH) && !k
								.equals(mxConstants.STYLE_DASHED)))
					ret += k + "=" + sh.get(k) + ";";
			}
		}
		if (outSourced)
