...
public class ImportExportPicker {
	...
	public class SortableFileFilter extends DefaultFileFilter implements
			Comparable<SortableFileFilter> {
	...
	public ImportExportPicker() {
		...
		ff = new SortableFileFilter(".mxe", "mxGraph Editor "
				+ mxResources.get("file") + " (.mxe)");
		add(ff, new MXEImportExport());
		ff = new SortableFileFilter(".scxml", "SCXML "
				+ mxResources.get("file") + " (.scxml)");
		defaultSaveFilter = ff;
		defaultOpenFilter = ff;
		add(ff, new SCXMLImportExport());
		ff = new SortableFileFilter(".html", "VML " + mxResources.get("file")
				+ " (.html)");
		add(ff, new VMLImportExport());
		ff = new SortableFileFilter(".html", "HTML " + mxResources.get("file")
				+ " (.html)");
		add(ff, new HTMLImportExport());
		ff = new SortableFileFilter(".svg", "SVG " + mxResources.get("file")
				+ " (.svg)");
		add(ff, new SVGImportExport());
		ff = new SortableFileFilter(".dot", "DOT " + mxResources.get("file")
				+ " (.dot)");
		...
		for (int i = 0; i < imageFormats.length; i++) {
			...
			ff = new SortableFileFilter("." + ext, ext.toUpperCase() + " "
	...
	public void addImportFiltersToFileChooser(JFileChooser fc) {
		ArrayList<SortableFileFilter> sortedFileFilters = new ArrayList<SortableFileFilter>(
				fileIO.keySet());
	...
	public IImportExport read(JFileChooser fc, SCXMLGraphEditor editor)
			throws Exception {
		...
		if (io == null) {
			...
			if (foundImporter == null)
				throw new IOException(
						"No importer available to read the selected file: "
								+ fileName);
			else
	...
	public void write(JFileChooser fc, SCXMLGraphEditor editor)
			throws Exception {
		String filename;
