...
public class SCXMLEdge implements Serializable {
	...
	public SCXMLEdge(String fromSCXMLID, List<String> toSCXMLIDs, String cond,
			String event, String content, HashMap<String, String> geometry) {
		edge = new HashMap<String, Object>();
	...
	public boolean isCycle() {
		...
		return (targets.size() == 1)
				&& (getSCXMLSource().equals(targets.get(0)));
	}
	...
	public mxGeometry getEdgeGeometry(String target) {
		HashMap<String, String> geometries = (HashMap<String, String>) edge
				.get(EDGEGEO);
		try {
			if (geometries != null) {
				...
				if (!SCXMLEditorStringUtils.isEmptyString(geometry)) {
					...
					while (m.find()) {
						points.add(new mxPoint(Double.parseDouble(m.group(1)),
								Double.parseDouble(m.group(2))));
					}
					m = SCXMLImportExport.offsetPattern.matcher(geometry);
					while (m.find()) {
						point = new mxPoint(Double.parseDouble(m.group(1)),
								Double.parseDouble(m.group(2)));
						offset = new mxPoint(Double.parseDouble(m.group(3)),
	...
	public String toString() {
		return getSCXMLSource() + "-[" + getCondition() + "," + getEvent()
				+ "]->" + getSCXMLTargets();
	}
	...
	public String getStyle(mxCell cell) {
		...
		// boolean
		// cycle=(cell==null)?isCycle():(cell.getTarget()==cell.getSource());
		// if (cycle && (!isCycleWithTarget())) ret+="strokeWidth=3;";

		SCXMLEdge edge = null;
		if (cell.isEdge())
			edge = (SCXMLEdge) cell.getValue();
		if (edge != null && edge.getEvent() != null
				&& !edge.getEvent().isEmpty())
			ret += "dashed=1;";
