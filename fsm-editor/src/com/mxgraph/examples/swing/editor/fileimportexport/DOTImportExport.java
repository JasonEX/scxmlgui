...
public class DOTImportExport implements IImportExport {
	...
	public void write(mxGraphComponent from, String into) throws IOException {
		...
		SCXMLEditorCell root = (SCXMLEditorCell) model.getRoot();
		if ((root != null) && (root.getChildCount() == 1))
			root = (SCXMLEditorCell) root.getChildAt(0);
		if (root != null) {
			String dot = mxVertex2DOTString(view, root, new StringBuffer(),
					true);
	...
	public SCXMLEditorCell getSimpleChildOf(SCXMLEditorCell n) {
		Queue<SCXMLEditorCell> list = new LinkedList<SCXMLEditorCell>();
		list.add(n);
		while (!list.isEmpty()) {
			SCXMLEditorCell x = list.poll();
			int l = x.getChildCount();
			for (int i = 0; i < l; i++) {
				SCXMLEditorCell c = (SCXMLEditorCell) x.getChildAt(i);
				if (c.isVertex()) {
					if (c.hasAVertexAsChild())
						list.add(c);
					else
						return c;
				}
	...
	private String mxVertex2DOTString(mxGraphView view, SCXMLEditorCell n,
			StringBuffer transitions, boolean isRoot) {
		...
		for (int i = 0; i < nc; i++) {
			SCXMLEditorCell c = (SCXMLEditorCell) n.getChildAt(i);
			if (c.isVertex()) {
				ret += mxVertex2DOTString(view, c, transitions, false);
			} else {
				SCXMLEditorCell nSource = (SCXMLEditorCell) c.getSource(), nTarget = (SCXMLEditorCell) c
						.getTarget();
				boolean sourceIsCluster = nSource.hasAVertexAsChild();
				boolean targetIsCluster = nTarget.hasAVertexAsChild();
				String source = (sourceIsCluster) ? "cluster_"
						+ nSource.getId() : null;
				String target = (targetIsCluster) ? "cluster_"
						+ nTarget.getId() : null;
				String realStart = (sourceIsCluster) ? realStart = getSimpleChildOf(
						nSource).getId()
						: nSource.getId();
				String realEnd = (targetIsCluster) ? realEnd = getSimpleChildOf(
						nTarget).getId()
						: nTarget.getId();
				String edgeLabel;
				...
				String ltail = (source != null) ? "ltail=\"" + source + "\""
						: null;
				String lhead = (target != null) ? "lhead=\"" + target + "\""
						: null;
				transitions.append("\n" + realStart + " -> " + realEnd + " ["
						+ ((ltail != null) ? ltail + "," : "")
						+ ((lhead != null) ? lhead + "," : "") + " label=\""
						+ edgeLabel + "\"];");
			}
		}
		if (isRoot)
			ret += transitions.toString();
		ret += close;
		return ret;
	}
