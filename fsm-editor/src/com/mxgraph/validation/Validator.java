...
public class Validator extends Thread {
	...
	public Validator(mxGraphComponent gc) {
		...
		graph.getModel().addListener(SCXMLEditorEvent.REQUEST_VALIDATION,
				new mxIEventListener() {
					public void invoke(Object sender, mxEventObject evt) {
						mxCell rootCell = (mxCell) evt.getProperty("root");
						if (!requests.contains(rootCell)) {
							System.out
									.println("Graph validation requested starting from cell: "
											+ rootCell);
	...
	private void processOldestRequest() {
		if (!requests.isEmpty()) {
			...
			model.fireEvent(new mxEventObject(SCXMLEditorEvent.VALIDATION_PRE_START));

			validateGraph(cell, context, warnings);

			model.fireEvent(new mxEventObject(SCXMLEditorEvent.VALIDATION_DONE,
					"warnings", warnings));
		}
	}
	 ...
	 */
	public boolean validateGraph(Object cell,
			Hashtable<Object, Object> context,
			HashMap<Object, String> totalWarnings) {
		...
		for (int i = 0; i < childCount; i++) {
			...
			if (warn != null) {
				...
				int len = html.length();
				graphComponent.setCellWarning(tmp,
		...
		if (warningsForCell == null)
			warningsForCell = "";

		// Adds error for invalid children if collapsed (children invisible)
		if (graph.isCellCollapsed(cell) && !isValid)
			warningsForCell += mxResources.get("containsValidationErrors",
					"Contains Validation Errors") + "\n";

		// Checks edges and cells using the defined multiplicities
		String tmp = null;
		if (model.isEdge(cell))
			tmp = graph.getEdgeValidationError(cell,
					model.getTerminal(cell, true),
					model.getTerminal(cell, false));
		else
			tmp = graph.getCellValidationError(cell);
		if (tmp != null)
			warningsForCell += tmp + "\n";
		...
		warningsForCell = SCXMLEditorStringUtils.cleanupSpaces(warningsForCell);
		if (!SCXMLEditorStringUtils.isEmptyString(warningsForCell))
			totalWarnings.put(cell, warningsForCell);
